...
const string shader_path = "./shaders/IntroScene_ES/";
const string objects_path = "./data/";

 IntroScene::IntroScene(QWidget *parent) :
     QGLWidget(parent), dist(12),ang_x(0),ang_y(0), widgetWidth(1), widgetHeight(1)
 {
 }
 ...
 {
     setup_gl();
//     shader.init(shader_path, "simple.vert", "", "simple.frag");
 ...
 void IntroScene::set_light_and_camera(ShaderProgramDraw& shader_prog)
 {
     ...
     shader_prog.set_projection_matrix(perspective_Mat4x4f(55, float(widgetWidth)/widgetHeight, 1, 100));
 ...
 {
     float delta_ang_x = 4*M_PI*(mouse_x0-m->x())/widgetWidth;
     float delta_ang_y = M_PI*(mouse_y0-m->y())/widgetHeight;
     ang_x += delta_ang_x;
     ang_y += delta_ang_y;
 }
