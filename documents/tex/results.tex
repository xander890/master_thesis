...
\begin{tabular}{p{3cm}l|l|l|l|l|}
\cline{3-6}
                             &      & \multicolumn{4}{c|}{Type of light}                                          \\ \cline{3-6} 
Model                        & \#$\Delta$& \multicolumn{1}{c|}{Point light} & \multicolumn{1}{c|}{Directional light} & \multicolumn{1}{c|}{With choice} & \multicolumn{1}{c|}{No loop} \\ \hline
\multicolumn{1}{|l|}{Dragon}  & $10^5$ & \mycolor{60}.5                  & \mycolor{60}.3                 & \mycolor{61}.0                 & \mycolor{58}.2                 \\ \hline
\end{tabular}
...
\begin{figure}
\centering
  \includegraphics[width=\linewidth]{images/matlab/multiple_lights_test.pdf}
\label{fig:numblights}
\caption{Rendering times for the environment lighting of figure TODO, split into the various components. The rendering times of step 2 are not shown, but they are $[87.2\ 90.2\ 89.4\ 91.6\ 95.2]$. We can see that the rendering time spent rendering the lights into the lightmap is increasing, while the other values are stable.}
\end{figure}
