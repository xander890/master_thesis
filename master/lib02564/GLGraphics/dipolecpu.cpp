...
Vec3f DipoleCPU::S_infinite(Vec3f _x, Vec3f _w, Vec3f _r, Vec3f _no)
{
    ...
    Vec3f _firstTerm = C_s * (powVec3f(_r,2) * _D_rev + 3 * _oneplus * dot_x_w);

    Vec3f _intermediate = (3 * _oneplus* invertVec3f(powVec3f(_r,2)) + material.transmissionCoefficient) * 3 * _D * dot_x_w;
...
void DipoleCPU::calculate2x2Texture(float inclinationDegreesFromNormal, std::vector<Vec3f> &texture, int textureSize)
{
    ...
    for(int i = 0; i < textureSize; i++)
    {
        ...
        for(int j = 0; j < textureSize; j++)
        {
            ...
            Vec3f r = M_PI * S_finite(xi,wi,xo,ni,no);
            float minR = -6.0f;
            float maxR =  0.0f;
            texture[i * textureSize + j] = r;//(-minR+log10(r))/abs(maxR-minR);
            cout << "[" << xincm << "," << yincm  << "] " << r << endl;
        }
    }
}
...
Vec3f DipoleCPU::S_finite(Vec3f _xi,Vec3f _wi,Vec3f _xo, Vec3f _nin, Vec3f _no)
{
    ...
    if (mu > 0)
    {
        ...
        _dr = (_rsquare + _D * mu * (_D * mu + 2 * _de * sqrtVec3f((pow(r,2) - pow(dot(_x,_w12), 2))*_rsqPlusDesqInv)));
    }
    else
    {
        Vec3f _redInv = invertVec3f(material.reducedExtinctionCoefficent);
        _dr = (_rsquare + 1 / 9 * _redInv * _redInv);
    }
    _dr = sqrtVec3f(_dr);
